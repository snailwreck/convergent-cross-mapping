        ...
        if window_len < 100:
            ...
            with st.spinner(f"Running Convergent Cross Mapping on {v1} and {v2} for t={window_start} to t={window_end}..."):
                ...
                try:
                    ...
                    st.markdown(f"Because Granger Causality relies on linear regression, we can extract the exact **Restricted (Univariate)** and **Unrestricted (Bivariate)** models generated by `statsmodels` and plot their predictions.")

                    # Add a slider to let the user pick which lag's model to inspect
                    viz_lag = st.slider("Select Lag to Visualize the Linear Fit", min_value=1, max_value=max_lag, value=1)

                    # gc_12 is the dictionary returned by the test.
                    # Key is the lag. The value is a tuple: (stats_dict, models_tuple)
                    # models_tuple = (restricted_model, unrestricted_model, contrast_matrix)
                    models_tuple = gc_12[viz_lag][1]
                    restricted_model = models_tuple[0]    # Univariate AR (Past Y only)
                    unrestricted_model = models_tuple[1]  # Bivariate AR (Past Y + Past X)

                    # The actual target values used in the regression (the function drops the first 'lag' rows)
                    ...
                    # Time axis shifted by the lag to match lengths
                    time_axis = df_lorenz_window['Time'].iloc[viz_lag:].values

                    fig_fit, ax_fit = plt.subplots(figsize=(10, 4))

                    # Plot only a slice so the user can actually see the lines distinctly (e.g., 150 points)
                    plot_limit = min(150, len(time_axis))

                    ax_fit.plot(time_axis[:plot_limit], actual_target[:plot_limit], label=f"Actual {v2}", color='black', lw=2)
                    ax_fit.plot(time_axis[:plot_limit], pred_restricted[:plot_limit], label=f"Univariate AR (Uses past {v2} only)", color='red', linestyle='dashed', alpha=0.8)
                    ax_fit.plot(time_axis[:plot_limit], pred_unrestricted[:plot_limit], label=f"Bivariate AR (Uses past {v2} & {v1})", color='dodgerblue', linestyle='dotted', lw=3)

                    ax_fit.set_xlabel("Time (Index)")
                    ax_fit.set_ylabel(v2)
                    ax_fit.set_title(f"Granger Linear Fits at Lag {viz_lag} (Zoomed in on first {plot_limit} points)")
                    ax_fit.legend()
